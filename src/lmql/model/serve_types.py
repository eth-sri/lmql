...
class LMQLInferenceAPIHandler(BaseHTTPRequestHandler):
    ...
    def client_id(self) -> str:
        if self._client_id is None:
            self.error_bad_request(
                msg="client_id not set (please provide the client_id in POST payload or in query."
            )
    ...
    def do_GET_results(self):
        ...
        while not self.state.client_results_queues[self.client_id].empty():
            ...
            # omit last colon
            if self.state.client_results_queues[self.client_id].empty():
                break
            else:
                self.wfile.write(b",")
        self.wfile.write(b"]")
    ...
    def do_queue_forward(self, payload, sample_id):
        try:
            ...
            if model_identifier != self.state.model_identifier:
                self.error_bad_request(
                    "The inference API serves model {} not {}.".format(
                        self.state.model_identifier, model_identifier
                    )
                )
                return
            ...
            assert (
                type(input_ids) == list
            )  # and all([type(i) == int for i in input_ids])
