...
function render_var(element, cursor_state) {
    ...
    if (var_name == "" && element.content == ":replay") {
        element.content = {button: "↺ Replay"}
    }
...
function render_text(s, cursor_state) {
    ...
    if (s.button) {
        return `<button class="promptdown-button-replay">${s.button}</button>`;
    }
...
function pd(el, config = null) {
    ...
    if (config.min_height) {
        el.style.minHeight = config.min_height;
    }
...
window.addEventListener('load', function() {
    this.document.querySelectorAll('.promptdown').forEach(el => {
        ...
        if (el.hasAttribute("min-height")) {
            config.min_height = el.getAttribute("min-height");
        }
